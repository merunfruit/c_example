...
void AudioMixer_queueSound(wavedata_t *pSound)
{
	...
	if(!hasFreeSlot) {
		printf("ERROR: No free slots in soundBites\n");
		return;
	}
}
...
// Change AudioMixer volume with constraints
void AudioMixer_changeVolume(int newVolume) {
	int volumeToChange = newVolume;
	if(volumeToChange < 0) {
		volumeToChange = 0;
	}
	if(volumeToChange > AUDIOMIXER_MAX_VOLUME) {
		volumeToChange = AUDIOMIXER_MAX_VOLUME;
	}

	AudioMixer_setVolume(volumeToChange);
}
...
static void fillPlaybackBuffer(short *playbackBuffer, int size)
{
	...
	pthread_mutex_lock(&audioMutex);
	// Wipe playbackBuffer
	memset(playbackBuffer, 0, size*2);

	for(int i = 0; i < MAX_SOUND_BITES; i++) {
		wavedata_t *waveData = soundBites[i].pSound;
		if(soundBites[i].pSound != NULL) {
			short *pData = waveData->pData;
			int offset = soundBites[i].location;
			int numSample = waveData->numSamples;
			_Bool isDoneSample = false;

			for(int j = 0; j < size; j++) {
				short buffer = playbackBuffer[j];
				short data = pData[offset];

				// Clip sound data
				short clipRes = 0;
				if(buffer > 0 && data > SHRT_MAX - buffer) {
					clipRes = SHRT_MAX; // Overflow
				}
				else if(buffer < 0 && data < SHRT_MIN - buffer) {
					clipRes = SHRT_MIN; // Underflow
				}
				else {
					clipRes = buffer + data;
				}

				// Add sound bite data to playback buffer
				playbackBuffer[j] = clipRes;
//				printf("%d | BUFFER IS %d | DATA IS %d | RESULT IS %d | BUFFER HAS %d\n ", j, buffer, data, clipRes, playbackBuffer[j]);

				// Increment location
				offset++;

				//Check if entire sample played
				if(offset == numSample) {
					isDoneSample = true;
					break;
				}
